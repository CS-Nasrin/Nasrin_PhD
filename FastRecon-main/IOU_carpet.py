...
def save_iou_results_to_excel(iou_results):
    ...
    excel_path = os.path.join(args.project_root_path, "iou_score95_7000.xlsx")
...
def remove_border_components(mask_np):
    """
    Removes connected components touching the image border.

    Args:
        mask_np (numpy.ndarray): Binary mask of shape (H, W).

    Returns:
        numpy.ndarray: Binary mask without border-touching components.
    """
    # Label connected components
    #labeled_mask, num_labels = label(mask_np, connectivity=2, return_num=True)
    labeled_mask = label(mask_np, connectivity=2)
    num_labels = labeled_mask.max() 

    # Create a mask for non-border components
    mask_cleaned = np.copy(mask_np)

    # Iterate through labeled components
    for region in regionprops(labeled_mask):
        # Check if component touches any border
        minr, minc, maxr, maxc = region.bbox
        if minr == 0 or minc == 0 or maxr == mask_np.shape[0] or maxc == mask_np.shape[1]:
            mask_cleaned[labeled_mask == region.label] = 0  # Remove border component
...
def keep_large_components(mask_np, min_area):
    ...
    large_components = np.where(component_sizes >= min_area)[0] + 1  
    large_components_mask = np.isin(labeled_mask, large_components)  

    return large_components_mask.astype(np.uint8)
...
def compute_iou(gt_mask, anomaly_binary):
    ...
    # New Cleaning Steps
    anomaly_binary = remove_border_components(anomaly_binary)  # Remove outer noise
    anomaly_binary = keep_large_components(anomaly_binary, min_area= 7000)  # Keep large defects only
...
class STPM(pl.LightningModule):
    ...
    def test_step(self, batch, batch_idx):
        ...
        anomaly_binary = keep_large_components(anomaly_binary,7000)

        # **Compute IoU with largest component filtering**
        ...
        binary_mask_save_path = os.path.join(args.project_root_path, 'masks_0.95_7000')
...
def get_args():
    ...
    parser.add_argument('--coreset_sampling_ratio', default=0.04)  # 0.01
    parser.add_argument('--project_root_path',
                        default=r'./IOU_bottle')  # root directory for saving logs and results
    ...
    parser.add_argument('--lambde', type=float, default=0.8) #Lambda regularization parameter for controlling model weight updates
