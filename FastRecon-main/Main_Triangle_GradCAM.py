...
def save_iou_results_to_excel(iou_results):
    df = pd.DataFrame(iou_results)  
    excel_path = os.path.join(args.project_root_path, "iou_scores95.1350.xlsx")
    df.to_excel(excel_path, index=False)
    print(f"IoU scores saved to {excel_path}")

def save_binary_mask(binary_mask, file_name, defect_type, save_path):
    """Save the binary mask as an image with defect type included in the filename."""
    os.makedirs(save_path, exist_ok=True)
    mask_filename = f"{defect_type}_{file_name}_binary.png"
...
def keep_large_components(mask_np, min_area):
    """
    Retains only the connected components in a binary mask that have an area >= min_area.

    Args:
        mask_np (numpy.ndarray): Binary mask of shape (H, W).
        min_area (int): Minimum area threshold for components to be kept.

    Returns:
        numpy.ndarray: Binary mask with only large connected components retained.
    """
    labeled_mask, num_labels = label(mask_np)  # Find connected components

    if num_labels == 0:
        return mask_np  # Return original if no components found

    # Count the size of each component
    component_sizes = np.bincount(labeled_mask.ravel())[1:]  # Ignore background (index 0)

    if len(component_sizes) == 0:
        return np.zeros_like(mask_np)  # No valid component

    # Get components that meet the area threshold
    large_components = np.where(component_sizes >= min_area)[0] + 1  # +1 because labels start from 1
    large_components_mask = np.isin(labeled_mask, large_components)  # Keep only large components

    return large_components_mask.astype(np.uint8)  # Convert back to uint8


def compute_iou(gt_mask, anomaly_binary):
    """
    Compute the Intersection over Union (IoU) score.
    Args:
        gt_mask (numpy array): Ground truth binary mask.
        anomaly_binary (numpy array): Predicted binary anomaly mask.
    Returns:
        float: IoU score.
    """
    if gt_mask is None or anomaly_binary is None:
        print("Error: Ground truth mask or predicted anomaly mask is None.")
        return 0.0
    
    if np.all(anomaly_binary == 0):
        return 1.0
    
    # Ensure the masks have the same size
    if gt_mask.shape != anomaly_binary.shape:
        anomaly_binary = cv2.resize(anomaly_binary, (gt_mask.shape[1], gt_mask.shape[0]), interpolation=cv2.INTER_NEAREST)
    

    # Flatten both arrays to compute IoU
    gt_flat = gt_mask.flatten()
    pred_flat = anomaly_binary.flatten()
...
class STPM(pl.LightningModule):
    def __init__(self, hparams):
        ...
        self.model = torch.hub.load('pytorch/vision:v0.9.0', 'resnet152', pretrained=True)

        target_layer = self.model.layer4[-1]  # Last convolutional block of ResNet-152
    ...
    def train_dataloader(self):
        image_datasets = MyDataset(root=os.path.join(args.dataset_path, args.category),
                                      transform=self.data_transforms, phase='train')
        print(f"Loaded {len(image_datasets)} train images.")
        train_loader = DataLoader(image_datasets, batch_size=args.batch_size, shuffle=True,
                                  num_workers=4)  # , pin_memory=True)
        return train_loader

    def test_dataloader(self):
        test_datasets = MyDataset(root=os.path.join(args.dataset_path, args.category),
    ...
    def training_step(self, batch, batch_idx):  # save locally aware patch features
        ...
        for feature in features:
            #m = torch.nn.AvgPool2d(3, 1, 1)   # With Avgpool 
    ...
    def test_step(self, batch, batch_idx):
        """
        Runs the anomaly detection test step and computes IoU with largest connected component filtering.
        """
        print('start')
        ...
        # Max value
        score = float(max(score_patches))
        anomaly_map_resized = cv2.resize(anomaly_map, (args.input_size[0], args.input_size[1]))
        anomaly_map_resized_blur = gaussian_filter(anomaly_map_resized, sigma=4)

        # Convert anomaly map to binary mask
        threshold = np.percentile(anomaly_map_resized_blur, 95)
        anomaly_binary = (anomaly_map_resized_blur > threshold).astype(np.uint8)

        # Load ground truth mask
        defect_type = x_type[0]
        gt_mask_path = os.path.join(args.dataset_path, args.category, 'ground_truth', defect_type, f'{file_name[0]}_mask.png')

        if os.path.exists(gt_mask_path):
            gt_mask = cv2.imread(gt_mask_path, cv2.IMREAD_GRAYSCALE)
            gt_mask = (gt_mask > 0).astype(np.uint8) if gt_mask is not None else np.zeros((args.input_size[1], args.input_size[0]), dtype=np.uint8)
        else:
            print(f"No ground truth found for {file_name[0]} in {defect_type}. Assuming black ground truth.")
            gt_mask = np.zeros((args.input_size[1], args.input_size[0]), dtype=np.uint8)

        anomaly_binary = keep_large_components(anomaly_binary,1350)


        # largest component filtering
        iou_score = compute_iou(gt_mask, anomaly_binary)
        print(f"IoU score for {file_name[0]}: {iou_score}")

        self.iou_results.append({"File Name": f"{defect_type}_{file_name[0]}", "IoU Score": iou_score})

        # Save filtered binary mask
        binary_mask_save_path = os.path.join(args.project_root_path, '22_0.95_1350')
        save_binary_mask(anomaly_binary, file_name[0], defect_type, binary_mask_save_path)


       # -------- Grad-CAM Starts Here --------
        x_input_tensor = x.detach().clone().requires_grad_(True)  # no need .to(self.device), already on correct device

        targets = [ClassifierOutputTarget(1)]  # You may need to replace 1 if your anomaly class index is different
        grayscale_cam = self.cam(input_tensor=x_input_tensor, targets=targets)[0]
        grayscale_cam = cv2.resize(grayscale_cam, (args.input_size[0], args.input_size[1]))

        rgb_img = x_input_tensor.squeeze().permute(1, 2, 0).cpu().numpy()
        rgb_img = min_max_norm(rgb_img)
        cam_image = show_cam_on_image(rgb_img, grayscale_cam, use_rgb=True)

        cam_save_path = os.path.join(self.sample_path, f"{x_type[0]}_{file_name[0]}_gradcam.jpg")
        ...
        save_iou_results_to_excel(self.iou_results)
...
def get_args():
    ...
    parser.add_argument('--dataset_path', default='/Users/nasrin/Downloads/FastRecon-main/new_triangle_analyze')
    parser.add_argument('--category', default='parts')
    parser.add_argument('--num_epochs', default=1)
    parser.add_argument('--batch_size', default=8)
    parser.add_argument('--load_size', default=[368, 368])  
    parser.add_argument('--input_size', default=[368, 368])  
    parser.add_argument('--coreset_sampling_ratio', default=0.05)  
    parser.add_argument('--project_root_path',
                        default=r'./ResNet152/Triangle_documentation')  # root directory for saving logs and results
                                           
    parser.add_argument('--save_src_code', default=False)
    parser.add_argument('--save_anomaly_map', default=True)
    parser.add_argument('--n_neighbors', type=int, default=0.3)
    parser.add_argument('--k', type=int, default=8) 
    parser.add_argument('--lambde', type=float, default=0.3) #Lambda regularization parameter for controlling model weight updates
    parser.add_argument('--error', type=float, default=0.1) 
    parser.add_argument('--train_num', type=float, default=8) 
