...
# def compute_iou(gt_mask, pred_heatmap, threshold=0.7):
#     """
#     Compute the Intersection over Union (IoU) score.
#     Args:
#         gt_mask (numpy array): Ground truth binary mask.
#         pred_heatmap (numpy array): Predicted heatmap.
#         threshold (float): Threshold to binarize the heatmap.
#     Returns:
#         float: IoU score.
#     """
#     if gt_mask is None or pred_heatmap is None:
#         print("Error: Ground truth mask or predicted heatmap is None.")
#         return 0.0
#     # Resize the heatmap to match the ground truth mask
#     pred_heatmap_resized = cv2.resize(pred_heatmap, (gt_mask.shape[1], gt_mask.shape[0]))
    
#     # Normalize and threshold the heatmap
#     pred_binary = (pred_heatmap_resized > threshold).astype(np.uint8)
...
class STPM(pl.LightningModule):
    ...
    def test_step(self, batch, batch_idx):  # Nearest Neighbour Search
        ...
        # Convert anomaly map to binary mask for IoU calculation
        threshold = np.percentile(anomaly_map_resized_blur, 90)  # Adjust threshold as needed
        ...
        self.pred_list_px_lvl.extend(anomaly_binary.ravel())
        self.pred_list_img_lvl.append(score)
        self.img_path_list.extend(file_name)

      # Construct ground truth mask path based on defect type
        defect_type = x_type[0]  # Extract defect type from batch
        gt_mask_path = os.path.join(args.dataset_path, args.category, 'ground_truth', defect_type, f'{file_name[0]}_mask.png')
        
        print(f"Looking for ground truth at: {gt_mask_path}")
        
        if os.path.exists(gt_mask_path):
            gt_mask = cv2.imread(gt_mask_path, cv2.IMREAD_GRAYSCALE)
            if gt_mask is None:
                print(f"Error: Could not read ground truth mask for {file_name[0]}. Assuming black ground truth.")
                gt_mask = np.zeros((args.input_size[1], args.input_size[0]), dtype=np.uint8)
            else:
                gt_mask = (gt_mask > 0).astype(np.uint8)  # Convert to binary mask
        else:
        ...
        # Save binary mask with defect type in filename
        binary_mask_save_path = os.path.join(args.project_root_path, 'binary_masks_0.90')
...
def get_args():
    ...
    parser.add_argument('--project_root_path',
                        default=r'./test_2layers_bottle_analyze')  # root directory for saving logs and results
    ...
    parser.add_argument('--lambde', type=float, default=0.1) #Lambda regularization parameter for controlling model weight updates
