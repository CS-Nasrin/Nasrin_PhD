...
class Personalized_mvtec_encoder(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.per_image_tokens and np.random.uniform() < self.mixing_prob:
            text = random.choice(imagenet_dual_templates_small).format(placeholder_string, str(per_img_token_list[idx % self.num_images]))
        else:
...
class Personalized_mvtec_mask(Dataset):
    ...
    def __getitem__(self, i):
        example = {}
        placeholder_string = str(self.placeholder_token)
        if self.coarse_class_text:
            placeholder_string = f"{str(self.coarse_class_text)} {placeholder_string}"

        if self.per_image_tokens and np.random.uniform() < self.mixing_prob:
            text = random.choice(imagenet_dual_templates_small).format(placeholder_string, str(per_img_token_list[i % self.num_images]))
        else:
            text = random.choice(imagenet_templates_small).format(placeholder_string)
...
class Positive_sample_with_generated_mask(Dataset):
    ...
    def __getitem__(self, i):
        ...
        # mask = self.flip(mask)
        image = np.array(image).astype(np.uint8)
        print("image shape before permute:", image.shape)
        if self.random_mask:
