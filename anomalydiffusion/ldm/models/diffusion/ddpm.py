...
class DDPM(pl.LightningModule):
    ...
    def get_input(self, batch, k):
        ...
        print("Final input image shape:", x.shape)
...
class LatentDiffusion(DDPM):
    ...
    def __init__(self,
        ...
        self.spatial_encoder_and_embedding=False
    ...
    def prepare_spatial_encoder(self,optimze_together=False,data_enhance=False,l=4):
        if optimze_together:
            self.spatial_encoder_and_embedding=True
            self.spatial_encoder =True
        self.embedding_manager.prepare_spatial_encoder(l)
        if data_enhance:
            from utils import random_transform
            self.data_enhance=random_transform()
    def prepare_test_dataset(self,sample_name,anomaly_name):
    ...
    def get_learned_conditioning(self, c,x=None,name=None):
        position=None
    ...
    def get_input(self, batch, k, return_first_stage_outputs=False, force_c_encode=False,
        ...
        if self.model.conditioning_key is not None:
            ...
            if cond_key != self.first_stage_key:
                if cond_key in ['caption', 'coordinates_bbox']:
                    xc = batch[cond_key] #执行这条：xc=['a rendition of the *', 'a close-up photo of the *', 'the photo of a *']
                ...
                xc = x
            if not self.cond_stage_trainable or force_c_encode: #不执行
                if isinstance(xc, dict) or isinstance(xc, list):
                    # import pudb; pudb.set_trace()
                    c,position = self.get_learned_conditioning(xc,x=masked_img,name=name)
                    total_dict['position']=position
                else:
                    c,position = self.get_learned_conditioning(xc.to(self.device),x=masked_img,name=name)
            else: #执行这条
                c = xc
            if bs is not None: #不执行
                c = c[:bs]

            if self.use_positional_encodings:#不执行
        ...
        out = [z, c,total_dict]
        if return_first_stage_outputs:#不执行
            xrec = self.decode_first_stage(z)
            out.extend([x, xrec])
        if return_original_cond:#不执行
            out.append(xc)
        return out
    ...
    def decode_first_stage(self, z, predict_cids=False, force_not_quantize=False):
        ...
        if hasattr(self, "split_input_params"): #不执行
            ...
            if isinstance(self.first_stage_model, VQModelInterface): #False 不执行
                return self.first_stage_model.decode(z, force_not_quantize=predict_cids or force_not_quantize)
            else:
                return self.first_stage_model.decode(z)
    ...
    def shared_step(self, batch, **kwargs):
        x, c,total_dict = self.get_input(batch, self.first_stage_key)
    ...
    def forward(self, x, c, *args,mask=None,mask_cond=None, name=None,**kwargs):
        ...
        return self.p_losses(x, c, t, *args,mask=mask, **kwargs)
    ...
    def apply_model(self, x_noisy, t, cond, return_ids=False,mask=None,weight_map=None,**kwargs):
        ...
        if hasattr(self, "split_input_params"):#不执行
            ...
            if self.cond_stage_key in ["image", "LR_image", "segmentation",
                ...
                adapted_cond,_ = self.get_learned_conditioning(adapted_cond)
                print(adapted_cond.shape)
            ...
            x_recon = self.model(x_noisy, t, **cond,mask=mask,weight_map=weight_map,**kwargs)
    ...
    def p_losses(self, x_start, cond, t,mask=None, noise=None):
        ...
        #x_start=torch.zeros_like(x_start)*mask+(1-mask)*x_start
        x_noisy = self.q_sample(x_start=x_start, t=t, noise=noise)
        model_output = self.apply_model(x_noisy, t, cond,mask=mask)
        ...
        if self.parameterization == "x0":
            ...
            raise NotImplementedError()
        if mask is not None:
            # model_output=model_output*(1-mask)
    ...
    #@torch.no_grad()
    def log_images(self, batch, N=8, n_row=4, sample=True, unconditional_only=False,ddim_steps=200, ddim_eta=1., return_keys=None,
                   quantize_denoised=True, inpaint=True, plot_denoise_rows=False, plot_progressive_rows=False,
                   plot_diffusion_rows=False,adaptive_mask=False, **kwargs):
        ...
        if sample:
            ...
            if quantize_denoised and not isinstance(self.first_stage_model, AutoencoderKL) and not isinstance(
                ...
                with self.ema_scope("Plotting Quantized Denoised"):
                    ...
                    # samples, z_denoise_row = self.sample(cond=c, batch_size=N, return_intermediates=True,
                    #                                      quantize_denoised=True)
                x_samples = self.decode_first_stage(samples.to(self.device),total_dict['mask'])
    ...
    def optimize_sample(self,z,source_img,mask):
        z.requires_grad_(True)
        for param in self.first_stage_model.parameters():
            param.requires_grad = True
        optimizer_z=torch.optim.Adam([z],lr=0.002)
        L1_loss=torch.nn.L1Loss(reduce=False)
        mask = (mask > 0.5).int()
        gen_img0 = self.decode_first_stage_with_grad(z).detach()
        for i in range(100):
            gen_img=self.decode_first_stage_with_grad(z)
            recon_loss=((1-mask)*L1_loss(gen_img,source_img)).mean()
            anomalay_loss=(mask*L1_loss(gen_img,gen_img0)).mean()
            loss = recon_loss + anomalay_loss
            #loss.requires_grad_(True)
            optimizer_z.zero_grad()
            print("total_loss=%.4f,recon_loss:%.4f,anomaly_loss:%.4f"%(loss,recon_loss,anomalay_loss))
            loss.backward()
            optimizer_z.step()
        return gen_img
    def configure_optimizers(self):
        lr = self.learning_rate
        if self.spatial_encoder_and_embedding:
            # params = [{'params': self.embedding_manager.embedding_parameters(), 'lr': lr},
    ...
    def on_save_checkpoint(self, checkpoint):
        ...
        if self.test_dataset:
            ...
            save_dir='save_models/%s/%s/%s'%(sub_dir,self.sample_name,self.anomaly_name)
            os.makedirs('save_models/%s/%s'%(sub_dir,self.sample_name),exist_ok=True)
            os.makedirs('save_models/%s/%s/%s'%(sub_dir,self.sample_name,self.anomaly_name),exist_ok=True)
            self.embedding_manager.save(os.path.join(save_dir, "embeddings.pt"))
            self.embedding_manager.save(os.path.join(save_dir, f"embeddings_gs-{self.global_step}.pt"))
            return
        if os.path.isdir(self.trainer.checkpoint_callback.dirpath):
            if self.spatial_encoder:
                torch.save(self.embedding_manager.spatial_encoder_model.state_dict(),os.path.join(self.trainer.checkpoint_callback.dirpath, "spatial_encoder.pt"))
...
class DiffusionWrapper(pl.LightningModule):
    ...
    def forward(self, x, t, c_concat: list = None, c_crossattn: list = None,**kwargs):
        ...
        elif self.conditioning_key == 'crossattn':#这个
