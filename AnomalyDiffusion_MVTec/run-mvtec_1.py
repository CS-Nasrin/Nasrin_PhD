    ...
    if os.path.isdir(os.path.join(root_dir,dir1)):
        ...
        for dir2 in dirs:
            if dir2!='good':
                ...
                os.system('set CUDA_VISIBLE_DEVICES=%d && python train_mask.py --mvtec_path="%s" --base configs/latent-diffusion/txt2img-1p4B-finetune.yaml -t --actual_resume ./models/ldm/text2img-large/model.ckpt  -n test --gpus 0, --init_word crack --sample_name=%s --anomaly_name=%s'%(opt.gpu_id,opt.data_path,dir1,dir2))
                os.system(
                    'set CUDA_VISIBLE_DEVICES=%d && python generate_mask.py --data_root="%s" --sample_name=%s --anomaly_name=%s --data_root="%s"'%(opt.gpu_id,opt.data_path,dir1,dir2,root_dir))
                if opt.adaptive_mask:
                    os.system(
                        'set CUDA_VISIBLE_DEVICES=%d && python generate_with_mask.py --data_root="%s" --sample_name=%s --anomaly_name=%s --adaptive_mask'%(opt.gpu_id,opt.data_path,dir1,dir2))
                else:
                    os.system(
                        'set CUDA_VISIBLE_DEVICES=%d && python generate_with_mask.py --data_root="%s" --sample_name=%s --anomaly_name=%s' % (
